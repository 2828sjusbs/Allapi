...
// ============ GENERATE KEY WITH SINGLE SELECT (FIXED) ============
app.post('/admin/generate-key', requireAuth, (req, res) => {
    ...
    // Check if custom key is enabled (checkbox)
    const isCustomEnabled = req.body.enable_custom === 'on' || req.body.enable_custom === true || req.body.enable_custom === 'true';
    
    console.log('📝 Generating key - Custom enabled:', isCustomEnabled);
    console.log('📝 Custom key value:', custom_key);
    console.log('📝 Selected API:', selected_api);
    
    // If custom key is enabled but no key provided
    ...
    function createKey(apiKey, isCustom) {
        ...
        const rateLimitEnabled = isUnlimited ? 0 : (rate_limit_enabled === 'on' || rate_limit_enabled === 'true' || rate_limit_enabled === '1' ? 1 : 0);
        const noteEnabled = note_enabled === 'on' || note_enabled === 'true' || note_enabled === '1' ? 1 : 0;
        const noteText = noteEnabled ? (key_note || '') : '';
        
        // Insert into database with api_enabled = 1 by default
...
app.post('/admin/edit-key', requireAuth, (req, res) => {
    ...
    if (name !== undefined) { updateFields.push('name = ?'); updateValues.push(name || 'Unnamed'); }
    ...
    db.run(
        ...
        function(err) {
            ...
            res.json({ 
                success: true, 
                api_enabled: enabled,
                message: enabled ? 'API Enabled' : 'API Disabled'
            });
        }
    );
});
...
app.all('/api/:endpoint', globalLimiter, async (req, res) => {
    ...
    db.get('SELECT * FROM api_keys WHERE key = ?', [userKey], async (err, keyData) => {
        ...
        // ✅ INDIVIDUAL API TOGGLE CHECK
        if (keyData.api_enabled === 0) {
            return res.json({
                success: false,
                error: 'This API Key has been disabled by the administrator.'
            });
        }
...
app.listen(PORT, () => {
    ...
    console.log('✅ INDIVIDUAL API TOGGLE WORKING!');
    console.log('✅ CUSTOM KEY GENERATION FIXED!');
    console.log('=====================================\n');
});
