...
db.serialize(() => {
    ...
    // Rate limit tracking with composite unique index
...
app.post('/admin/generate-key', requireAuth, (req, res) => {
    ...
    function createKey(apiKey, isCustom) {
        ...
        if (selected_api) {
            allowedApisJson = selected_api === 'all' ? '["all"]' : JSON.stringify([selected_api]);
        }
...
app.post('/admin/edit-key', requireAuth, (req, res) => {
    const { 
        ...
        rate_limit_enabled, rate_limit_per_day, rate_limit_per_hour, rate_limit_per_minute,
        note_enabled, key_note, status, selected_api, custom_expiry_date, custom_expiry_time, api_enabled
    } = req.body;
...
// ============ MAIN API ENDPOINT (WITH RATE LIMIT VISIBILITY) ============
...
app.listen(PORT, () => {
    ...
    console.log('✅ TOGGLE API FIXED');
    console.log('✅ RATE LIMIT VISIBILITY & AUTOMATIC LOCK WORKING!');
    console.log('=====================================\n');
});
